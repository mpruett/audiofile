...
static void testFLAC(int fileFormat, int channelCount, int sampleWidth, int frameCount)
{
	...
	ASSERT_TRUE(file);
	ASSERT_EQ(afGetCompression(file, AF_DEFAULT_TRACK), AF_COMPRESSION_FLAC);
...
TEST(FLAC, FLAC_16)
{
	for (int channelCount=1; channelCount<=4; channelCount++)
		testFLAC<int16_t>(AF_FILE_FLAC, channelCount, 16, 82421);
}

TEST(FLAC, FLAC_24)
{
	for (int channelCount=1; channelCount<=4; channelCount++)
		testFLAC<int32_t>(AF_FILE_FLAC, channelCount, 24, 82421);
}
