...
void ALAC::initDecoder()
{
	...
	size_t inputPacketBytes =
		_af_format_frame_size_uncompressed(&m_track->f, false) *
		m_track->f.framesPerPacket +
		kALACMaxEscapeHeaderBytes;
	m_readBuffer = new Buffer(inputPacketBytes);
	BitBufferInit(&m_bitBuffer,
		static_cast<uint8_t *>(m_readBuffer->data()),
		inputPacketBytes);
}
...
void ALAC::runPull()
{
	...
	size_t bytesPerPacket = packetTable->bytesPerPacket(m_currentPacket);

	read(m_readBuffer->data(), bytesPerPacket);

	uint32_t numFrames;
	m_decoder->Decode(&m_bitBuffer, static_cast<uint8_t *>(m_outChunk->buffer),
...
void ALAC::runPush()
{
	...
	m_encoder->Encode(inputFormat, outputFormat(),
